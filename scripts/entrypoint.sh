...
: "${ORDS_USER:?ORDS_USER no definido}"            # normalmente 'APEX_PUBLIC_USER'
: "${ORDS_PWD:?ORDS_PWD no definido}"
: "${ORDS_CONFIG:?ORDS_CONFIG no definido}"        # ej: /etc/ords/config (volumen)
...
# ========== derivar APEX_SCHEMA desde APEX_VERSION ==========
# 24.1  -> APEX_240100
# 24    -> APEX_240000
...
# ========== “sentry” por si accidentalmente copiaron YAML aquí ==========
...
SELECT open_mode FROM v$PDBS WHERE CON_ID = (SELECT con_id FROM v$session WHERE audsid = USERENV('SESSIONID'));
EXIT;
EOF
do
  log "PDB aún no está READ WRITE. Reintento en 20s..."
  sleep 20
done
log "DB lista."
...
if ! sql_sys <<EOF | grep -q "1"
SET HEADING OFF FEEDBACK OFF
SELECT 1 FROM dba_users WHERE username = UPPER('${APEX_SCHEMA}');
EXIT;
EOF
then
...
# ========== ORDS: descargar si no existe ==========
if [ ! -x /opt/oracle/ords/bin/ords ]; then
  log "Descargando ORDS (WAR) y expandiendo…"
...
if [ -d "${APEX_HOME}/images" ]; then
  log "Copiando imágenes estáticas de APEX a /usr/local/tomcat/webapps/i …"
